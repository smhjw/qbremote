...
class QbRepository : TorrentBackend {
    ...
    override suspend fun fetchGlobalSpeedLimits(): Result<com.hjw.qbremote.data.model.GlobalSpeedLimits> = runCatching {
        val prefJson = executeWithRetry { liveApi -> liveApi.getPreferences() }
        val obj = JsonParser.parseString(prefJson).asJsonObject
        com.hjw.qbremote.data.model.GlobalSpeedLimits(
            downloadLimit = obj.get("dl_limit")?.asLong ?: 0L,
            uploadLimit = obj.get("up_limit")?.asLong ?: 0L,
            altDownloadLimit = obj.get("alt_dl_limit")?.asLong ?: 0L,
            altUploadLimit = obj.get("alt_up_limit")?.asLong ?: 0L,
            useAltSpeedLimits = obj.get("use_alt_speed_limits")?.asBoolean ?: false,
            schedulerEnabled = obj.get("scheduler_enabled")?.asBoolean ?: false,
            scheduleFromHour = obj.get("schedule_from_hour")?.asInt ?: 8,
            scheduleFromMin = obj.get("schedule_from_min")?.asInt ?: 0,
            scheduleToHour = obj.get("schedule_to_hour")?.asInt ?: 20,
            scheduleToMin = obj.get("schedule_to_min")?.asInt ?: 0,
            schedulerDays = obj.get("scheduler_days")?.asInt ?: 0,
        )
    }

    override suspend fun setGlobalSpeedLimits(
        downloadLimit: Long,
        uploadLimit: Long,
        altDownloadLimit: Long,
        altUploadLimit: Long,
        ...
        val prefs = JsonObject().apply {
            addProperty("dl_limit", downloadLimit)
            addProperty("up_limit", uploadLimit)
            addProperty("alt_dl_limit", altDownloadLimit)
            addProperty("alt_up_limit", altUploadLimit)
            addProperty("scheduler_enabled", schedulerEnabled)
            addProperty("schedule_from_hour", scheduleFromHour)
            addProperty("schedule_from_min", scheduleFromMin)
            addProperty("schedule_to_hour", scheduleToHour)
            addProperty("schedule_to_min", scheduleToMin)
            addProperty("scheduler_days", schedulerDays)
        }
        val response = executeWithRetry { liveApi ->
            liveApi.setPreferences(prefs.toString())
        }
        response.ensureSuccess("Set global speed limits failed.")
    }

    override suspend fun toggleAltSpeedMode(): Result<Boolean> = runCatching {
        val response = executeWithRetry { liveApi -> liveApi.toggleSpeedLimitsMode() }
        response.ensureSuccess("Toggle alt speed mode failed.")
        val modeText = executeWithRetry { liveApi -> liveApi.getSpeedLimitsMode() }
        modeText.trim() == "1"
    }
    ...
    override suspend fun addTorrent(request: AddTorrentRequest): Result<Unit> = runCatching {
        ...
        if (request.files.isEmpty() && urlsText.isNotBlank()) {
            ...
            checkAddTorrentResponse(response)
            return@runCatching
        }
                ...
                try {
                    ...
                    if (loginResult.ipBanned) {
                        throw AuthLockedException(
                            "IP banned due to too many failed login attempts. Unban in qB WebUI or wait for the ban to expire. Detail: ${loginResult.detail}"
                        )
                    }
    ...
    private fun parseLoginResponse(
        ...
        val loginOk = response.isSuccessful &&
            (loginText.equals("Ok.", ignoreCase = true) || response.code() == 204)
        if (loginOk) {
            val okDetail = if (response.code() == 204) "204 No Content" else "Ok."
            return LoginResult(success = true, detail = "$endpointLabel -> $okDetail")
        }
    ...
    /**
     * 检查 addTorrent 响应。
     * qB 5.2.0+ 返回 JSON: {"added_torrent_ids":[...],"failure_count":0,...}
     * 旧版返回纯文本 "Ok." 或 "Fails."
     */
    private fun checkAddTorrentResponse(response: Response<String>) {
        response.ensureSuccess("Add torrent failed.")
        val resultText = response.body().orEmpty().trim()
        if (resultText.isBlank()) return

        val parsed = runCatching { JsonParser.parseString(resultText).asJsonObject }.getOrNull()
        if (parsed != null) {
            val failCount = parsed.get("failure_count")?.asInt ?: 0
            if (failCount > 0) {
                throw IllegalStateException("Failed to add torrent: $resultText")
            }
        } else if (resultText.contains("fail", ignoreCase = true)) {
            throw IllegalStateException("Failed to add torrent: $resultText")
        }
    }
    ...
    private fun normalizeTransferInfo(
        ...
        val serverState = maindata?.getAsJsonObject("server_state")
        val freeSpaceOnDisk = serverState?.get("free_space_on_disk")?.asLong ?: transfer.freeSpaceOnDisk
        val useAltSpeedLimits = serverState?.get("use_alt_speed_limits")?.asBoolean ?: transfer.useAltSpeedLimits
        if (downloaded == transfer.downloadedTotal && uploaded == transfer.uploadedTotal
            && freeSpaceOnDisk == transfer.freeSpaceOnDisk && useAltSpeedLimits == transfer.useAltSpeedLimits
        ) {
            return transfer
        }
